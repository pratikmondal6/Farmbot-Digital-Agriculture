...
describe('Plant Routes', () => {
  ...
  test('POST /api/plant/save with valid depth', async () => {
    const res = await request(app)
      .post('/api/plant/save')
      .send({ plantType: 'Lettuce', valueType: 'depth', value: 10 }); // positive value
  ...
  test('POST /api/plant/save with invalid depth', async () => {
    const res = await request(app)
      .post('/api/plant/save')
      .send({ plantType: 'Lettuce', valueType: 'depth', value: 50 }); // out of range
  ...
  test('POST /api/plant/add-type with valid data', async () => {
    const res = await request(app)
      ...
      .send({ plant_type: 'TestPlant', minimal_distance: 100, seeding_depth: 10 }); // positive value
    expect(res.statusCode).toBe(201);
  ...
  test('POST /api/plant/save-depth with valid data', async () => {
    const res = await request(app)
      ...
      .send({ plantType: 'TestPlant', depth: 15 }); // positive value
  ...
  test('POST /api/plant/save-depth with invalid depth', async () => {
    const res = await request(app)
      .post('/api/plant/save-depth')
      .send({ plantType: 'TestPlant', depth: 50 }); // out of range
    expect(res.statusCode).toBe(400);
