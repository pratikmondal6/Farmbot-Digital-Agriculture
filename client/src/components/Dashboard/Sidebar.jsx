    ...
    return (
        <aside className="dashboard-sidebar" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
            <nav className="sidebar-nav">
                ...
                <button
                  className={getClass("soilHumidityPage")} onClick={() => { onSelectComponent("soilHumidityPage"); setActiveClass("soilHumidityPage");}}> 
                    💧 {(activeClass === "dashboard" || isHovered) && <span>Soil Humidity</span>}
                </button>
            </nav>
        </aside>
    );
